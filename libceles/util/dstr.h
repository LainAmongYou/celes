...
struct dstr {
	char  *array;
	size_t size; /* number of characters, excluding null terminator */
	size_t capacity;
};
...
static inline void dstr_clear(struct dstr *dst);
...
EXPORT void dstr_ncopy(struct dstr *dst, const char *array, const size_t size);
EXPORT void dstr_ncopy_dstr(struct dstr *dst, const struct dstr *src, const size_t size);
...
EXPORT void dstr_ncat(struct dstr *dst, const char *array, const size_t size);
EXPORT void dstr_ncat_dstr(struct dstr *dst, const struct dstr *str, const size_t size);
...
static inline void dstr_copy_dstr(struct dstr *dst, const struct dstr *src)
{
	...
	if (src->size) {
		dstr_ensure_capacity(dst, src->size + 1);
		memcpy(dst->array, src->array, src->size + 1);
		dst->size = src->size;
	}
}
...
{
	if (capacity == 0 || capacity <= dst->size)
		return;
...
static inline void dstr_cat(struct dstr *dst, const char *array)
{
	...
	size = strlen(array);
	dstr_ncat(dst, array, size);
}

static inline void dstr_cat_ch(struct dstr *dst, char ch)
{
	dstr_ensure_capacity(dst, ++dst->size + 1);
	dst->array[dst->size - 1] = ch;
	dst->array[dst->size]     = 0;
}
...
static inline int dstr_cmp(const struct dstr *str1, const char *str2)
{
	return strcmp(!!str1->array ? str1->array : "", !!str2 ? str2 : "");
}
